                ...
                run: function(t) {
                    if (arbolCargado) {
							if(path!=null)
							{
								...
								if (arbol.selModel.selNode!=null){if(arbol.selModel.selNode.id==path[path.length-1]) {stop(this); path=null;}}
							}
                    } else {
                        contador++;
                        if (contador > 10) stop(); // si a los diez segundos no se ha cargado el árbol que pare la búsqueda
                        //if (arbol.selModel.selNode!=null){if(arbol.selModel.selNode.id==path[path.lenght]) {stop();  path=null;}}
                    }
                }
                ,
