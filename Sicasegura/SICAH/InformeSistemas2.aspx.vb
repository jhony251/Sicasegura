...
#Region " Web Form Designer Generated Code "
    ...
    Private Sub InitializeComponent()
        ...
        'egb 
        'Me.reportGenerator1.RenderDocument()
...
#End Region
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
        If Request.QueryString("idsistema") <> "" Then CargarReport()
    ...
    Private Sub LoadReportTemplate()
        reportGenerator1.LoadTemplate(Replace(Server.MapPath("ListadoSistemas.rst"), "sicah", "Listados"))
    ...
    Private Sub crearTablas()
        ...
        '------------------------------------------------------------------------------------------------
        'EGB 07/08/2008 Agregamos un nuevo dataset de calculo de Subtotales
        dst.Tables.Add("Subtotales")
        dst.Tables("Subtotales").Columns.Add(New DataColumn("Subtotal", Type.GetType("System.Decimal")))
        dst.Tables("Subtotales").Columns.Add(New DataColumn("SubtotalStr", Type.GetType("System.String")))
        dst.Tables("Subtotales").Rows.Add()
        dst.Tables("Subtotales").Rows(0).Item("Subtotal") = 0
        dst.Tables("Subtotales").Rows(0).Item("SubtotalStr") = ""
    ...
    Private Sub crearDataSets(ByVal codigoPVYCR As String, ByVal idElementoMedida As String, ByVal Tipo As String, ByVal Factor As String)
        ...
        End If
        '----------------------------------------------------------------------------------------------------------------------------------
        'EGB 07/08/2008 Formateo del Resultado
        dst.Tables("Subtotales").Rows(0).Item("SubtotalStr") = String.Format("{0:#,##0.0}", dst.Tables("Subtotales").Rows(0).Item("Subtotal"))
        '----------------------------------------------------------------------------------------------------------------------------------
    End Sub
                ...
                Case "V"    'motor                    
                    ...
                    Else
                        If IdIncidencia = 0 Then
                            'EGB 04/08/2008 Si no existen datos de una lectura siguiente (lectura1=0) impedimos que se le cambie el signo a Lectura0
                            'LINEA ORIGINAL
                            'dst.Tables("TablaAuxiliar").Rows(i).Item("Diferencial") = Lectura1 - Lectura0
                            '------------------------------------------------------------------------------
                            If Lectura1 = 0 Then
                                dst.Tables("TablaAuxiliar").Rows(i).Item("Diferencial") = Lectura0
                            Else
    ...
    Private Sub CalcularTotales(ByVal CodigoPVYCR As String, ByVal IdElementoMedida As String, ByVal Factor As Int32, ByVal Tipo As String)
        ...
        '--------------------------------------------------------------------------------------------------------------------------------------------------------
        'EGB 07/08/2008 Actualización del valor de Subtotales sobre el dataset de subtotales
        dst.Tables("Subtotales").Rows(0).Item("Subtotal") = dst.Tables("Subtotales").Rows(0).Item("Subtotal") + Math.Round(diferencial * Factor, 1)
        '--------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Private Function GetCalcularTotales(ByVal CodigoPVYCR As String, ByVal IdElementoMedida As String, ByVal Factor As Int32, ByVal Tipo As String) As Int32
        'EGB 07/08/2008 Se trata de el procedimiento anterior pero transformado en función 
        'para actualizar el DataSet de Subtotales desde su invocación
        dst.Tables("Listados").Rows.Add()
        dst.Tables("Listados").Rows(indice).Item("CodigoPVYCR") = CodigoPVYCR
        dst.Tables("Listados").Rows(indice).Item("IdElementoMedida") = IdElementoMedida
        dst.Tables("Listados").Rows(indice).Item("Factor") = Factor
        dst.Tables("Listados").Rows(indice).Item("VolumenDif") = String.Format("{0:#,##0.0}", Math.Round(diferencial, 1))
        dst.Tables("Listados").Rows(indice).Item("VxF") = String.Format("{0:#,##0.0}", Math.Round(diferencial * Factor, 1))
        'EGB 07/08/2008 Actualización del dst.Tables("Subtotal")
        '-----------------------------------------------------------------------------------------------------------------------------------------------------
        GetCalcularTotales = dst.Tables("Listados").Rows(indice).Item("VxF")
        '-----------------------------------------------------------------------------------------------------------------------------------------------------
        dst.Tables("Listados").Rows(indice).Item("Tipo") = Tipo
        If Factor > 0 Then
            dst.Tables("Listados").Rows(indice).Item("Color") = ""
    ...
    Private Function CalcularIncidencias(ByVal i As Integer, ByVal IdIncidencia As Integer) As Double
        ...
        Return dif
    End Function
    Protected Sub CargarReport()

        If Request.QueryString("idsistema") = "" Then
    ...
    Protected Sub btnCaudalesFiltradosI_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCaudalesFiltradosI.Click
        ...
        If Session("idsistema") = "" Then
            Exit Sub
        End If
        Response.Redirect("../sicah/InformeSistemas2.aspx?idsistema=" & Session("idsistema"))
        'If ddlSistema.SelectedValue = "" Then
        '    Alert(Me, "Debe seleccionar un sistema para lanzar el informe")
        '    Exit Sub
        'End If
