...
Partial Class SICAH_sistemas
    ...
    Private Sub crearDataSets()
        ...
        sentenciaOrder = " order by NumInscripcion "
    ...
    Protected Sub btnAceptar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAceptar.Click
        ...
        Try
            If lblIdSel.Text = "" Then
                ...
                If (Me.txtModifConcesion.Text <> "") Then
                    comando.CommandText = "Select max(id) from [SICA_SIST_ModificaCon]"
                    maxId = utiles.nullACero(comando.ExecuteScalar)
                    comando.CommandText = "INSERT INTO [SICA_SIST_ModificaCon]  (ID, NumInscripcion,Volumen,Year) VALUES (" & maxId + 1 & ", @ID_Sistema,@Volumen, @Year) "
                    comando.Parameters.AddWithValue("ID_Sistema", utiles.BlancoANull(Me.txtNumInscripcion.Text))
                    comando.Parameters.AddWithValue("Volumen", utiles.BlancoANull(Me.txtModifConcesion.Text.Replace(",", ".")))
                    comando.Parameters.AddWithValue("Year", utiles.BlancoANull(Date.Now.Year.ToString))
                    comando.ExecuteNonQuery()
                End If
            ...
            Else
                ...
                comando.CommandText = "Select count(*) from [SICA_SIST_ModificaCon] where NumInscripcion like '" & utiles.BlancoANull(Me.txtNumInscripcion.Text) & "' and Year like'" & utiles.BlancoANull(Date.Now.Year.ToString) & "'"
                Dim existe As Integer
                existe = utiles.nullACero(comando.ExecuteScalar)

                comando.CommandText = "UPDATE [SICA_SIST_ModificaCon] SET  " & _
                                      " Volumen=@Volumen where NumInscripcion like '" & utiles.BlancoANull(Me.txtNumInscripcion.Text) & "' and Year like'" & utiles.BlancoANull(Date.Now.Year.ToString) & "'"
                comando.Parameters.AddWithValue("Volumen", utiles.BlancoANull(Me.txtModifConcesion.Text.Replace(",", ".")))
                comando.ExecuteNonQuery()
        ...
        'ipim 16/03/2012: Esto es para que se lanze lo de la configuración del sistema en el momento en que se da de alta.
        If lblIdSel.Text <> "" Then
            'Esto es lo uqe había antes
            lblIdSel.Text = ""
            crearDataSets()
    ...
    Protected Sub btnCancelar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCancelar.Click
        ...
        lblIdSel.Text = ""
        crearDataSets()
        Me.lblTitulo.Text = "CONFIGURACIÓN DE SCV:"
    End Sub
        ...
        Select Case e.CommandName
            ...
            Case "borrar"
                ...
                Try
                    ...
                    comando.CommandText = "SELECT NumInscripcion from [SICA_SIST_Sistemas] where id=" & lblIdSel.Text
                    Dim inscripcion As Integer
                    inscripcion = comando.ExecuteScalar
                    ...
                    For i As Integer = 0 To dst.Tables("TablaPuntosSistema").Rows.Count - 1
                        ...
                        comando.ExecuteNonQuery()
                    Next
                    'También borramos la relación con los usuarios
                    comando.CommandText = "Delete [SICA_SIST_Sistemas-Usuarios] where ID_Sistema=" & lblIdSel.Text
                    comando.ExecuteNonQuery()
                    'Y al final el sistema
                    comando.CommandText = "Delete [SICA_SIST_Sistemas] where id=" & lblIdSel.Text
                    comando.ExecuteNonQuery()

                    comando.CommandText = "Delete [SICA_SIST_modificacon] where NumInscripcion =" & inscripcion
                    comando.ExecuteNonQuery()
    ...
    Protected Sub cargarControlesEdicion()
        If dst.Tables("TablaAgrupaciones").Rows.Count > 0 Then
            ...
            If dst.Tables("TablaAgrupaciones").Rows(0).Item("Nombre") Is System.DBNull.Value Then
                ...
                comando.CommandText = "Select volumen from [SICA_SIST_ModificaCon] where NumInscripcion like '" & utiles.BlancoANull(Me.txtNumInscripcion.Text) & "' and Year like'" & utiles.BlancoANull(Date.Now.Year.ToString) & "'"
                Me.txtModifConcesion.Text = utiles.nullACero(comando.ExecuteScalar)
