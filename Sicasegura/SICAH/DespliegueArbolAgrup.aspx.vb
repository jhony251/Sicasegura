...
Partial Class SICAH_DespliegueArbolAgrup
    ...
    Protected Sub CargarRamasHijos(ByVal nodo As String, ByVal idarbolPadre As String)
        ...
        For Each fila In dstPuntos.Tables("TablaHijos").Rows
            ...
            Dim nodotexto, nodoclave As String
            If (fila("EM").ToString().Contains("Q")) Then
                nodotexto = "" & fila("CodigoPVYCR").ToString().Trim() & "-" & fila("EM").ToString().Trim() & "+-+CAUDAL"
                nodoclave = "" & fila("CodigoPVYCR").ToString().Trim() & ";E;Q"
            End If
            If (fila("EM").ToString().Contains("V")) Then
                nodotexto = "" & fila("CodigoPVYCR").ToString().Trim() & "-" & fila("EM").ToString().Trim() & "+-+VOLUMEN"
                nodoclave = "" & fila("CodigoPVYCR").ToString().Trim() & ";E;V"
            End If
            If (fila("EM").ToString().Contains("H")) Then
                nodotexto = "" & fila("CodigoPVYCR").ToString().Trim() & "-" & fila("EM").ToString().Trim() & "+-+HORAS"
                nodoclave = "" & fila("CodigoPVYCR").ToString().Trim() & ";E;H"
            End If
            If (fila("EM").ToString().Contains("E")) Then
                nodotexto = "" & fila("CodigoPVYCR").ToString().Trim() & "-" & fila("EM").ToString().Trim() & "+-+ENERGÍA"
                nodoclave = "" & fila("CodigoPVYCR").ToString().Trim() & ";E;E"
            End If

            hijos &= ",{id: 'W" & fila("ID_PuntoSistema") & "',text: '" & fila("CodigoPVYCR") & "',cls:'CauceNivel2Tree'," & _
            "href:'InformacionGeneral.aspx?nodotexto=" & nodotexto.Trim() & "&nodoclave=" & nodoclave.Trim() & "&x=3',hrefTarget:'iframeDetalle',leaf:'true'"
    ...
    Protected Sub CargarRamasRaices()
        ...
        'Primer nivel
        Dim Str As String = "{id:'P0',text: 'Inscripciones RACCS',cls:'CauceNivel0Tree'},{id:'P1',text: 'Otras agrupaciones',cls:'CauceNivel0Tree'}"
    ...
    Private Function RamasVisiblesString(ByVal Nodo As String)
        ...
        If Nodo = "0" Then
            ...
            For Each fila In dstAgrupaciones.Tables("TablaAgrup").Rows
                str = str & ",{id: '" & fila("ID") & "',text: '" & fila("NumInscripcion") & " - " & fila("Descripcion") & "',cls:'CauceNivel1Tree'," & _
