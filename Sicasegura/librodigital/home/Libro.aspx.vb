...
Partial Class Informes
    ...
    Dim conexion_pruebas As SqlClient.SqlConnection = New System.Data.SqlClient.SqlConnection(ConfigurationSettings.AppSettings("dsn"))
    Dim conexion_dbsica As SqlClient.SqlConnection = New System.Data.SqlClient.SqlConnection(ConfigurationSettings.AppSettings("dsn_dbsica"))
    Dim conexion_ld As SqlClient.SqlConnection = New System.Data.SqlClient.SqlConnection(ConfigurationSettings.AppSettings("dsn_ld"))
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        informe = "Capt2_tuberia"
        'Depende del tipo de captación que sea se muestra un informe u otro. 
        If informe <> "" Then
            reportGenerator1.LoadTemplate(Server.MapPath("plantillas_informes\" & informe & ".rst"))
    ...
    Private Sub crearDataSets()
        ...
        If informe.EndsWith("tuberia") Then
            da_dbsica.SelectCommand.CommandText &= " WHERE left(EM,1)<>'Q'"
        Else
            da_dbsica.SelectCommand.CommandText &= " WHERE left(EM,1)='Q'"
        End If

        ''
        ''EN EL CASO DE QUE ESTEMOS ACCEDIENDO A ESTE INFORME DESDE AGRUPACIONES.
        ''
        If (NumInscripcion > 0) Then

            da_dbsica.SelectCommand.CommandText = "Select P.ID, P.CodigoPVYCR, P.EM, P.CFD, P.TOC, P.FI, P.FF " & _
                " From SICA_SIST_Sistemas AS S " & _
                " inner join [SICA_SIST_Sistemas-Punto] AS SP ON SP.ID_Sistema = S.ID " & _
                " inner join [SICA_SIST_PuntoSistema] AS P ON P.ID = SP.ID_PuntoSistema " & _
                " where (S.NumInscripcion =  " & NumInscripcion & ")"
        ...
        If dst.Tables("TablaPuntos").Rows.Count > 0 Then
            ...
            With dst.Tables("TablaPuntos").Rows(0)
                Select Case Left(informe, 6)
                    Case "Capt1_"
                        strSql = AccesoADatos.SacarLecturasPreYPro(.Item("em").ToString().Trim(), " WHERE CodigoPVYCR='" & .Item("codigopvycr").ToString().Trim() & "' AND idElementoMedida='" & .Item("em").ToString().Trim() & "' and cod_fuente_dato='10' ", _
                            "ORDER BY Fecha_Medida", conexion_pruebas)(1)
                    Case Else
                        strSql = AccesoADatos.SacarLecturasPreYPro(.Item("em").ToString().Trim(), " WHERE CodigoPVYCR='" & .Item("codigopvycr").ToString().Trim() & "' AND idElementoMedida='" & .Item("em").ToString().Trim() & "' and cod_fuente_dato='10' AND year(fecha_medida)>" & Today.Year - 1, _
                            "ORDER BY Fecha_Medida", conexion_pruebas)(1)
                End Select

                Dim db As New SicaSegura.SICA_DB

                Dim dtt As New DataTable
                dtt = db.GetDataSIGVECTOR(strSql)
                dtt.TableName = "Datos"
                dst.Tables.Add(dtt)
    ...
    Private Function CalculaPrimeraLectura(ByVal valor As Double, ByVal codigoPVYCR As String, ByVal EM As String, ByVal fechaMedida As DateTime) As Double
        ...
        strSql = AccesoADatos.SacarLecturasPreYPro(EM, " WHERE CodigoPVYCR='" & codigoPVYCR & "' AND idElementoMedida='" & EM & "' and cod_fuente_dato='19' AND Fecha_Medida<'" & fechaMedida & "' ", _
                                "order by fecha_medida desc", conexion_pruebas)(1)
        da_pruebas.SelectCommand.CommandText = strSql
        da_pruebas.Fill(dst, "DatosPrimeraLectura")
